...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Buttons
  var smart_submit_button = document.getElementById('smart_submit_button');
  var add_account_button = document.getElementById('add_account_button');
  var id_submit_button = document.getElementById('id_submit_button');
  var copy_button = document.getElementById('copy_button');
  var cancel_add_account = document.getElementById('cancel_add_account_button');
  var submit_new_account_button = document.getElementById('submit_new_account_button');
  var import_accounts_button = document.getElementById('import_accounts_button');
  
  // Inputs
  var import_accounts_file_input = document.getElementById('import_accounts_file_input');
  var mpassword_input = document.getElementById('mpassword_input');
  ...
  copy_button.addEventListener('click', function() {
    var mpassword = document.getElementById('copy_input');
  ...
  mpassword_input.addEventListener('keyup', function() {
    var input_number = document.getElementById('smart_number_input');
    ...
    if (validate_masterpw_hash( mpassword_input.value, 
      ...
      derive_password( mpassword_input.value, 
                        input_number.value, 
                        USER_ACCOUNTS_FILENAME );
    } else {
...
/**
 * Check if a match for current site is found in the list.
...
/**
 * Handles keyevents for ENTER, offered as an alternative way to confirm input.
 */
document.addEventListener("keyup", function(event) {
  event.preventDefault();
  var smart_number_panel = document.getElementById('smart_number_panel');
  var site_selector_panel = document.getElementById('site_selector_panel');
  var masterpw_panel = document.getElementById('masterpw_panel');
...
function load_user_account(USER_ACCOUNTS_FILENAME) {
  var user_acc_url = chrome.runtime.getURL(USER_ACCOUNTS_FILENAME);
  var file_content = load_json(user_acc_url);
...
function validate_masterpw_hash(mpassword, smart_number, USER_ACCOUNTS_FILENAME) {
  ...
  var sm_number_hash = sjcl.hash.sha256.hash(smart_number);
  var derived_pbkdf2 = sjcl.misc.pbkdf2(mpassword_hash, sm_number_hash, CYCLES, PW_LENGTH);
      derived_pbkdf2 = sjcl.codec.hex.fromBits(derived_pbkdf2); 
  
  // Verify with original hash
  user_acc_details = load_user_account(USER_ACCOUNTS_FILENAME);
 ...
 * @param {String} smart_number - smart number associated with account
 * @param {String} USER_ACCOUNTS_FILENAME
 * Uses external library: cry_trans1.js
 */
function derive_password(mpassword, smart_number, USER_ACCOUNTS_FILENAME) {
  const CYCLES = 10000;
  const PW_LENGTH = 256;
  const MOD_VALUE = 17;

  var site_name = document.getElementById('site_datalist');
  ...
  $('#masterpw_panel').fadeOut('slow', function() {
    ...
    var pw_hash    = sjcl.hash.sha256.hash(smart_number + mpassword + site_name.value);
    
    // 2) Concatenate smart number with account name, 
    //    and hash resulting string by sha256 
    var smart_hash = sjcl.hash.sha256.hash(smart_number + site_name.value);
...
function add_new_account() {
  var add_account_panel = document.getElementById('add_account_panel');
  save_account('');
...
function read_user_accounts(filename) {
  ...
  var site_array = fileContent.split('\n');
...
function transform_icon() {
  ...
  var icon_container_small = document.getElementById('icon_container_small');
...
function validate_smart_number() {
  var smart_number  = document.getElementById('smart_number_input');
  var error_message = document.getElementById("smart_number_error");
...
function validate_smart_hash(smart_input_value, USER_ACCOUNTS_FILENAME) {
  ...
  var accounts_json   = JSON.parse(file_content);
  var orig_smart_hash = accounts_json.smartnum;
  var input_hash      = sjcl.hash.sha256.hash(smart_input_value);
...
function save_current_url() {
  get_current_url((url) => {
      ...
      strip_url = url.replace(/(^\w+:|^)\/\//, '');
      url_array = strip_url.split(".");
